...
1. Modify device credentials - MQTT Basic
   ...
   `Login in ThingsBoard CE/PE as tenant` --> `Devices` -->  Click on *my device* --> `Details` --> `Manage credentials` -> Select credentials type: *MQTT Basic* -> Input client ID: *MY_CLIENT_ID* -> `Save`.
