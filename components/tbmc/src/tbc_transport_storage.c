...
#define FIELD_STRDUP(dest, src) \
                if (src) { \
                    dest = strdup(src);\
                }

#define FIELD_MEMDUP(dest, src, size) \
                if (src && size>0) { \
                    dest = malloc(size); \
                    if (dest) { \
                                             ...
                                             const tbc_transport_address_config_t *config)
{
    ...
    FIELD_STRDUP(storage->schema, config->schema);
    FIELD_STRDUP(storage->host, config->host);
    storage->port = config->port;
    FIELD_STRDUP(storage->path, config->path);
    return storage;
}
                                      ...
                                      const tbc_transport_credentials_config_t *config)
{
    ...
    FIELD_STRDUP(storage->client_id, config->client_id);
    FIELD_STRDUP(storage->username, config->username);
    FIELD_STRDUP(storage->password, config->password);
    FIELD_STRDUP(storage->token, config->token);
    return storage;
}
                                ...
                                const tbc_transport_verification_config_t *config)
{
    ...
    if (config->cert_len>0) {
        FIELD_MEMDUP(storage->cert_pem, config->cert_pem, config->cert_len);
    } else {
        FIELD_STRDUP(storage->cert_pem, config->cert_pem);
    }
                        ...
                        const tbc_transport_authentication_config_t *config)
{
    ...
    if (config->client_key_password_len>0) {
        FIELD_MEMDUP(storage->client_key_password, config->client_key_password, config->client_key_password_len); /*!< Client key decryption password string */
    } else {
        FIELD_STRDUP(storage->client_key_password, config->client_key_password); /*!< String length of the password pointed to by client_key_password */
    }
...
static void _transport_verification_storage_free_fields(tbc_transport_verification_storage_t *verification)
{
    ...
    FIELD_FREE(verification->cert_pem); /*!< Pointer to certificate data in PEM or DER format for server verify (with SSL), default is NULL, not required to verify the server. PEM-format must have a terminating NULL-character. DER-format requires the length to be passed in cert_len. */
...
static void _transport_authentication_storage_free_fields(tbc_transport_authentication_storage_t *authentication)
{
    ...
    FIELD_FREE(authentication->client_cert_pem);        /*!< Pointer to certificate data in PEM or DER format for SSL mutual authentication, default is NULL, not required if mutual authentication is not needed. If it is not NULL, also `client_key_pem` has to be provided. PEM-format must have a terminating NULL-character. DER-format requires the length to be passed in client_cert_len. */
    authentication->client_cert_len = 0;                /*!< Length of the buffer pointed to by client_cert_pem. May be 0 for null-terminated pem */
    FIELD_FREE(authentication->client_key_pem);         /*!< Pointer to private key data in PEM or DER format for SSL mutual authentication, default is NULL, not required if mutual authentication is not needed. If it is not NULL, also `client_cert_pem` has to be provided. PEM-format must have a terminating NULL-character. DER-format requires the length to be passed in client_key_len */
    authentication->client_key_len = 0;                 /*!< Length of the buffer pointed to by client_key_pem. May be 0 for null-terminated pem */
    FIELD_FREE(authentication->client_key_password);    /*!< Client key decryption password string */
